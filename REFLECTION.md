...
Process: I first created the utils.py as this is the foundation. This has to be first as you can't validate a student or save data until the tools are built. I then moved on to the app.py file which imported the utils and implements 'Person' and 'Student' classes with Inheritance and getters/setters. I also imported matplotlib to this file in order to track student progress and show how classes interact by adding Course Class. Matplotlib allowed me to create a visual representation of student progress using a graph. The last thing to do was to test the app file so I created the test_app.py file in order to do that. I added unit tests for ID validation and class properties which served to check if both the REGEX and the getters were working as they should. I then run all the programs and made sure they worked. As mentioned in the problem solving part there were a few things I had to solve.